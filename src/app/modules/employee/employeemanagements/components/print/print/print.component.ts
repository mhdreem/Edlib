...
export class PrintComponent implements OnInit {
  ...
  title4: string;
  title5: string;
  constructor(@Inject(MAT_DIALOG_DATA) public data: [TBLShamelEmployee, {punishmentsCheckbox: number, bonusCheckbox: number, holidaiesCheckbox: number}],
  ...
  private tblshamelPrintFooterService: TblshamelPrintFooterService) {
    ...
    this.tblshamelPrintFooterService.list(this.userId).subscribe(
      res =>{
        const printFooter= res.filter(printFooter => printFooter.form_name== "ManageEmployeePrintCardBack");
