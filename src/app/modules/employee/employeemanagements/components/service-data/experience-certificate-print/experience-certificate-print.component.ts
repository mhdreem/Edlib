...
type jobState={jobName: string, totalYears: number, totalMonths: number, totalDays: number}
@Component({
...
export class ExperienceCertificatePrintComponent implements OnInit {
  ...
  jobStates: jobState[]= [];
  constructor(@Inject(MAT_DIALOG_DATA) public data: [TBLShamelEmployee, any],
  ...
  private tblshamelPrintFooterService: TblshamelPrintFooterService) { 
    ...
    //   this.jobStates.forEach(jobState=>{
    //     if (jobState.jobName == TBLShamelSCJobState.TBLShamelJobName.jobname_name){
    //       jobState.totalDays= TBLShamelSCJobState.begindate
    //     }
    //   });
    // });
  }
