...
@Component({
  selector: 'app-employee-state-data-print',
  templateUrl: './employee-state-data-print.component.html',
  styleUrls: ['./employee-state-data-print.component.scss']
})
export class EmployeeStateDataPrintComponent implements OnInit {
  ...
  private tblshamelPrintFooterService: TblshamelPrintFooterService) {
    ...
    this.tblshamelPrintFooterService.list(this.userId).subscribe(
      res =>{
        const printFooter= res.filter(printFooter => printFooter.form_name== "PrintEmpState");
