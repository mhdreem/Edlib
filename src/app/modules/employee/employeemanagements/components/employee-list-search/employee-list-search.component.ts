...
export class EmployeeListSearchComponent implements OnInit {
  ...
  isListItemActive(activeId: number, listItemId: number): boolean {
    return activeId == listItemId
  }

  // setActiveButton<T>(key: keyof T) {
  //   Object.keys(this.employeeListSortOptions).map(item)
  // }
  ...
  SelectItemChange (Selected_Employee: ViewTBLShamelEmployee)
  {
    ...
    this.activeEmployeeId = Selected_Employee.id;
  ...
  OnInputEmployeeNameChange(value:any,sort:boolean =true, sortKey: any)
  {
