...
export function  Uniqe( ShamelShatebPunishmentService: TBLShamelShatebPunishmentService ) : AsyncValidatorFn
{
    return (FormAbstract: AbstractControl) :  Observable<ValidationErrors  | null > => 
    {
      ...
      return null;
        ...
        return  ShamelShatebPunishmentService.isUniqueRecord (Form.value.id,Form.value.value.documentnum,Form.value.startdate,Form.value.documentdate,Form.value.serial).
        pipe(map(
          (data:any ) => 
          {           
