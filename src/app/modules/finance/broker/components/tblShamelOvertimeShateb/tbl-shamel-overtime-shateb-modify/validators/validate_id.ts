...
export function  Validate_ID( ShamelOverTimeShatebService:TBLShamelOverTimeShatebService ) : AsyncValidatorFn
{
    return (formControl: AbstractControl) :  Observable<ValidationErrors>   => 
    {
      ...
      if (formControl.value == null ||  formControl.value == undefined || formControl.value.length ==0 )
        ...
        return ShamelOverTimeShatebService.SearchById (formControl.value).
        pipe(map(
          (data:any ) => 
          {           
