...
export class TblshamelsccourseService {
  ...
  Validate(obj:ITBLShamelSCCourse) : Observable<ITBLShamelSCCourse[]> {
    const headers = new HttpHeaders({'Content-Type': 'application/json; charset=utf-8'});
    const options = {  headers: headers };  
    return this.httpClient.post<ITBLShamelSCCourse[]>(this.RestUrl +"TBLShamelSCCourse/Validate",obj);      
  }
}
